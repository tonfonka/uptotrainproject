	...
	<!--<div class="blog-page blog-content-1">
     ...
     <?php
				if($count>0){
					for($i=0;$i<$count;$i++){
						$tripname = DB::table('trips')
							->select(['trips_name','triprounds.start_date','triprounds.departure_date','trips.id','booking.status','trips.image'])
							->join('triprounds','trips.id','=','triprounds.trip_id')
							->join('booking','triprounds.id','=','booking.tripround_id')
							->where('triprounds.id',$triproundbook[$i])
							->orderBy ('triprounds.start_date' , 'desc') 
							->get();
						...
						if(($tripname[0]->start_date)>=$today){	
							...
							echo 			'<div class="card">';
							echo          '<div class="card-image">'.'<img class="img-responsive" src="/images/'.($tripname[0]->image).'">';
							echo					'</div>';
